...
function users_get_data ($redirectOnError){
    ...
    if (!$email){
        flash('É necessário preencher o campo E-mail', 'error');
...
$users_create = function() use ($conn){
    ...
    if(is_null($data['password'])){
        flash('É necessário preencher o campo senha.', 'error');
...
$users_edit = function($id) use($conn){
    ...
    $sql = 'UPDATE users SET email=?, updated=NOW() WHERE idusers=?';

    if($data['password']){
        $data['password'] = password_hash($data['password'], PASSWORD_DEFAULT);
        $sql = 'UPDATE users SET email=?, password=?, updated=NOW() WHERE idusers=?';
    }
